...
parser.add_argument('--val_path', default='../ILSVRC2012_img_val', type=str)
parser.add_argument('--train_path', default='../../ILSVRC2012/', type=str)
...
def main():
    ...
    # create model
    if args.pretrained and args.dataset == 'ilsvrc':
        print("=> using pre-trained model '{}'".format(args.arch))
        model = models.__dict__[args.arch](pretrained=True)
    else:
        print("=> creating model '{}'".format(args.arch))
        if args.dataset == 'cifar10':
            model = vgg11(num_classes=10, fc_h = 1, fc_w = 128)
    ...
    logger_train = Logger('./logs/{}_{}/train'.format(args.dataset,args.suffix))
    logger_val = Logger('./logs/{}_{}/val'.format(args.dataset, args.suffix))
    ...
    logspace_lr = torch.logspace(-2, -4, args.epochs)

    if args.dataset == 'ilsvrc':
        # Data loading code
        traindir = args.train_path + 'ILSVRC2012_sampling_{}'.format(args.sample_rate)
        ...
        transform_test = transforms.Compose([
            ...
            transforms.Normalize(mean=[0.4914, 0.4822, 0.4465],  # mean
        ...
        val_dataset = datasets.CIFAR10(root='../data/CIFAR10', train=False,
        ...
        if args.sample_rate>0:
            sampler = np.load('CIFAR10_samplerate_{}.npy'.format(args.sample_rate))
            train_list = []
            for itm in sampler:
                train_list += itm.tolist()

            train_loader = torch.utils.data.DataLoader(
                train_dataset, batch_size=args.batch_size,
                num_workers=args.workers, pin_memory=True,  sampler=SubsetRandomSampler(train_list))
        elif args.sample_rate<0:
            sampler = np.load('CIFAR10_samplerate_0.5.npy')
            train_list = []
            for itm in sampler:
                train_list += np.random.permutation(itm)[:10].tolist()

            np.save('CIFAR10_poor_trainlist.npy', train_list)
        ...
        # Data loading code
        traindir = '../../CUB_bird_binary/train/'
        valdir = '../../CUB_bird_binary/val/'

        normalize = transforms.Normalize(mean=[0.4795, 0.4822, 0.4218],
        ...
        if args.sample_rate>0:
            sampler = np.load('CUB_samplerate_{}.npy'.format(args.sample_rate))
            train_list = []
            for itm in sampler:
                train_list += itm
            ...
            sampler = np.load('CUB_samplerate_0.5.npy')
            train_list = []

            subsampler_0 = np.random.permutation(sampler[0])[:24].tolist()
            subsampler_1 = np.random.permutation(sampler[1])[:24].tolist()

            train_list += subsampler_0*8
            train_list += subsampler_1*8

            np.save('trainlist_poor.npy', train_list)
