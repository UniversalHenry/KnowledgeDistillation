...
def main():
    ...
    tar_data = torch.load('/data/HaoChen/knowledge_distillation/Overfit/tar_pretrain__XX.pkl')
    print("Loaded inputFeature_pretrain !")
    print("Loading dW_pretrain ...")
    dw_data = torch.load('/data/HaoChen/knowledge_distillation/Overfit/dx_pretrain__XX.pkl')
    ...
    for filter_order in filter_show:
        ...
        for key in pca:
            ...
            dir = "./pretrain__XX/hist_no_0/" + key + "/"
            if not os.path.exists(dir):
                os.makedirs(dir)
        ...
        # print("Saving PCA...")
        # dir = "./pca_data/pretrain__XX/filter(%d,%d)/" % (filter_order + 1, filter_num)
        # if not os.path.exists(dir):
        #     os.makedirs(dir)
