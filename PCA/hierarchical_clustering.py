...
def main():
    ...
    while typeorder < typenum:
        ...
        for filter_order in filter_show:
            dir = "./Hierarchical_clustering_data/res_pretrain_%d/filter(%d,%d)/res/" % (
            ...
            k += 1
            # print("(%d/%d) ..." % (k, n),file=f)
            dir = "./Hierarchical_clustering_data/res_pretrain_%d/filter(%d,%d)/PCA/" % (
                typeorder, filter_order + 1, filter_num)
            if not os.path.exists(dir):
                os.makedirs(dir)
            ...
            while select_feature_num > 0:
                ...
                for i in np.arange(1,20):
                    ...
                    dir = "./Hierarchical_clustering_data/res_pretrain_%d/filter(%d,%d)/select_feature(%d)/" % ( typeorder ,filter_order + 1, filter_num ,select_feature_num)
                    if not os.path.exists(dir):
                        os.makedirs(dir)
