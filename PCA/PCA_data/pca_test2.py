...
def main():
    np.random.seed(0)
    filter_show = range(100)   # which filters to calculate
    interval = 100  # divide the histogram into how many parts
    # where to load the data
    print("Loading inputFeature_random ...")
    tar_data = np.abs(np.random.randn(1000,100,1,512,3,3)) / 2
    print("Loaded inputFeature_random !")
    print("Loading dW_random ...")
    dw_data = np.abs(np.random.randn(1000,100,512,3,3)) / 2
    ...
    for filter_order in filter_show:
        for order in range(num):
            for top_order in range(top_num):
                ...
                tar_x[filter_order][order * top_num + top_order] = tar_data[order][filter_order][top_order].reshape([1,-1])
                dw_x[filter_order][order] = dw_data[order][filter_order].reshape([1,-1])
        for top_order in range(top_num):
    ...
    for filter_order in filter_show:
        ...
        print("Saving PCA...")
        dir = "./pca_data/res_test2/filter(%d,%d)/" % (filter_order + 1, filter_num)
        if not os.path.exists(dir):
            os.makedirs(dir)
