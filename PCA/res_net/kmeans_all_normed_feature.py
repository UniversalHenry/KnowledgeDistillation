...
def main():
    ...
    for filter_order in filter_show:
        dir = "./Kmeans_data_normed_feature/res_pretrain_%s/filter(%d,%d)/res/" % (
        ...
        k += 1
        # print("(%d/%d) ..." % (k, n),file=f)
        dir = "./Kmeans_data_normed_feature/res_pretrain_%s/filter(%d,%d)/PCA/" % (
            'all_10', filter_order + 1, filter_num)
        if not os.path.exists(dir):
            os.makedirs(dir)
        ...
        while select_feature_num > 0:
            ...
            for i in np.arange(1,20):
                Kmeans_data_normed_feature[i] = {}
                Kmeans_data_normed_feature[i]['all'] = KMeans(n_clusters=i, random_state=0,max_iter=1000,tol=1e-8).fit(select_feature)
                print("Kmeans(n_clusters=",i,",inertia_=", Kmeans_data_normed_feature[i]['all'].inertia_, ",n_iter_=",
                      Kmeans_data_normed_feature[i]['all'].n_iter_,",select_feature=",select_feature_num,"):",file=f)
                print("Kmeans(n_clusters=", i, ",inertia_=" , Kmeans_data_normed_feature[i]['all'].inertia_ , ",n_iter_=",
                      Kmeans_data_normed_feature[i]['all'].n_iter_, ",select_feature=", select_feature_num, "):")
                # print(Kmeans_data_normed_feature[i]['all'].labels_)
                unique, counts = np.unique(Kmeans_data_normed_feature[i]['all'].labels_, return_counts=True)
                res = dict(zip(unique, counts))
                print(res,file=f)
                print(res)
                Kmeans_data_normed_feature[i]['res'] = res
                ...
                print("Saving Kmeans...")
                dir = "./Kmeans_data_normed_feature/res_pretrain_%s/filter(%d,%d)/select_feature(%d)/" % ( 'all_10' ,filter_order + 1, filter_num ,select_feature_num)
                if not os.path.exists(dir):
                    os.makedirs(dir)
                torch.save(Kmeans_data_normed_feature,dir + "Kmeans_%s_pretrain_filter(%d,%d).pth.tar" % ('all_10' ,filter_order + 1, filter_num))
                print("Kmeans saved.")
