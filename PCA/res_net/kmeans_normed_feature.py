...
def main():
    ...
    while typeorder < typenum:
        ...
        data[typeorder] = torch.load( dir + 'Processed_data_pretrain_' + str(typeorder) + '_v1_filter(0-9).pth.tar')
        print('Loaded data(' + str(typeorder + 1) + '/' + str(typenum) + ').')

        for filter_order in filter_show:
            dir = "./Kmeans_data_normed_feature/res_pretrain_%d/filter(%d,%d)/res/" % (
            ...
            pca_component["dot_res_norm"] = pca_component["dot_res_norm"] / np.mean(
                np.sum(pca_component["dot_res_norm"] ** 2, axis=1)) ** 0.5
            k += 1
            # print("(%d/%d) ..." % (k, n),file=f)
            dir = "./Kmeans_data_normed_feature/res_pretrain_%d/filter(%d,%d)/PCA/" % (
                typeorder, filter_order + 1, filter_num)
            if not os.path.exists(dir):
                os.makedirs(dir)
