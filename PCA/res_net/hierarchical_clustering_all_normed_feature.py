...
def main():
    ...
    for filter_order in filter_show:
        dir = "./Hierarchical_clustering_data_normaed_feature/res_pretrain_%s/filter(%d,%d)/res/" % (
        ...
        k += 1
        # print("(%d/%d) ..." % (k, n),file=f)
        dir = "./Hierarchical_clustering_data_normaed_feature/res_pretrain_%s/filter(%d,%d)/PCA/" % (
            'all_10', filter_order + 1, filter_num)
        if not os.path.exists(dir):
            os.makedirs(dir)
        ...
        while select_feature_num > 0:
            ...
            for i in np.arange(1, 20):
                ...
                print("Saving Hierarchical_clustering_data_normaed_feature...")
                dir = "./Hierarchical_clustering_data_normaed_feature/res_pretrain_%s/filter(%d,%d)/select_feature(%d)/" % (
