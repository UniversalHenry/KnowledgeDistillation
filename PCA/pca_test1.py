...
def main():
    ...
    tar_data = np.random.randn(1000,100,1,512,3,3)
    print("Loaded inputFeature_random !")
    print("Loading dW_random ...")
    dw_data = np.random.randn(1000,100,512,3,3)
    ...
    for filter_order in filter_show:
        ...
        print("Saving PCA...")
        dir = "./pca_data/res_test1/filter(%d,%d)/" % (filter_order + 1, filter_num)
        if not os.path.exists(dir):
            os.makedirs(dir)
