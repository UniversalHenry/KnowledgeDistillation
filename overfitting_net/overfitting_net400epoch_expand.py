...
def OF_net_train(gpu_id,datasets,sample_rate,net_id):
    depth = 56
    resume_dir = 'checkpoint_Reset(400expand){0}_{1}_samplerate_{2}_netid_{3}.pth.tar'.format(depth, datasets,sample_rate,net_id)
    best_prec1 = 0
    epoch_step = [60, 120, 160, 200, 240, 280, 320, 360] # list with epochs to drop lrN on
    num_classes = 10 if datasets == 'CIFAR10' else 100

    logger_train = Logger('./logs/%s-%s-%s-%s/train' % ('resnet(400expand)'+str(depth), datasets,'sample_rate'+str(sample_rate),'net_id'+str(net_id)))
    logger_val = Logger('./logs/%s-%s-%s-%s/val' % ('resnet(400expand)'+str(depth), datasets,'sample_rate'+str(sample_rate),'net_id'+str(net_id)))
