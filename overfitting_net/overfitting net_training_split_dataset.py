...
def main():
    ...
    for sample_rate in [0.2]:
        for net_id in range(1,5):
            OF_net_train(gpu_id,datasets,sample_rate,net_id)

def OF_net_train(gpu_id,datasets,sample_rate,net_id):
    depth = 56
    resume_dir = 'split_dataset_checkpoint_Reset{0}_{1}_samplerate_{2}_netid_{3}.pth.tar'.format(depth, datasets,sample_rate,net_id)
    best_prec1 = 0
    ...
    net = resnet(depth=56, num_classes=num_classes)
    net.cuda(gpu_id)
    ...
    trainset,label,origin_trainset = sample_choose(datasets,trainset,sample_rate,net_id)
...
def sample_choose(datasets,trainset,sample_rate,net_id):
    origin_trainset = trainset
    if net_id == 0:
        data = []
        data += [{'trainset_data':[],'label':[],'groundtruth':[],'id':i} for i in range(math.ceil(1/sample_rate))]
        data += [{'origin_trainset_data':trainset.train_data,'origin_label':trainset.train_labels}]
        for j in range(min(trainset.train_labels),max(trainset.train_labels)+1):
            ...
            jth_set = random.sample(jth_set, jth_set.__len__())
            interval = math.ceil(jth_set.__len__()*sample_rate)
            set_length = jth_set.__len__()
            for l in range(math.ceil(1/sample_rate)):
                data[l]['label'] += jth_set[interval*l:min(interval*(l+1),set_length)]
        for l in range(math.ceil(1/sample_rate)):
            data[l]['label'] = random.sample(data[l]['label'],data[l]['label'].__len__())
            data[l]['trainset_data'] += [trainset.train_data[i] for i in data[l]['label']]
            data[l]['groundtruth'] += [trainset.train_labels[i] for i in data[l]['label']]
        torch.save(data,"%s_split_dataset_info_sample_rate_%s" % (datasets,str(sample_rate)))

    data = torch.load("%s_split_dataset_info_sample_rate_%s" % (datasets,str(sample_rate)))
    trainset.train_labels = data[net_id]['groundtruth']
    trainset.train_data = data[net_id]['trainset_data']
    return trainset,data[net_id]['label'],origin_trainset
